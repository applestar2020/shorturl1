...
MyDB::MyDB()
{
    ...
    initDB("106.52.58.96", "apple", "star", "test");
}
...
string MyDB::find_surl_SQL(string lurl)
{
    //mysql_query()执行成功返回0,执行失败返回非0值。
    string sql = "SELECT hash FROM short_url_map where lurl='" + lurl + "';";
...
string MyDB::find_lurl_SQL(string surl)
{
    //mysql_query()执行成功返回0,执行失败返回非0值。
    string sql = "SELECT lurl FROM short_url_map where hash='" + surl + "';";
    if (mysql_query(mysql, sql.c_str()))
    {
        cout << "Query Error: " << mysql_error(mysql);
        return "-1";
...
bool MyDB::add_surl_SQL(string surl, string lurl)
{
    string sql = "INSERT INTO short_url_map (lurl, hash) VALUES ('" + lurl + "', '" + surl + "');";
